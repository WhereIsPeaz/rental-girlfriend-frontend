...
export default function BookingPage() {
    ...
    useEffect(() => {
        ...
        // Load service and provider data from API
        const loadData = async () => {
            try {
                setLoading(true)

                // Fetch service data
                const foundService = await getService(String(id))
                setService(foundService)

                // Fetch provider data
                const foundProvider = await getUser(foundService.providerId)
                        ...
                        <div className="rounded-2xl bg-white p-6 shadow-sm">
                            ...
                            <div className="flex items-center space-x-4">
                                ...
                                <div>
                                    ...
                                    <div className="text-sm text-gray-500">
                                        {
                                            provider.birthdate // ตรวจสอบว่า birthdate มีค่าหรือไม่
                                                ? `${new Date().getFullYear() - new Date(provider.birthdate).getFullYear()} ปี`
                                                : '? ปี' // แสดง '?' ถ้าไม่มีข้อมูล
                                        }
                                    </div>
