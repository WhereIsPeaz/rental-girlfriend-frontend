...
export default function FormActions({ formData, handleChange }: AccountFieldsProps) {
  ...
  const makeCheckboxEvent = (name: string, checked: boolean) =>
    ({ target: { name, type: "checkbox", checked } } as unknown as React.ChangeEvent<HTMLInputElement>);
    ...
    <div>
      <div className="flex items-start gap-2">
        <input
          ...
          required
        />
        <label htmlFor="terms" className="block text-sm text-[#212b36] leading-6">
          ฉันยอมรับ{" "}
          <Link
            href="#"
          ...
          และ{" "}
          <Link href="/privacy" className="font-medium text-[#f24472] hover:underline">
      ...
      <button
        type="submit"
        style={{ background: "linear-gradient(133.15deg, #F24BA7 2.02%, #EF4444 98.99%)", borderRadius: 6 }}
        className="mt-6 w-full cursor-pointer px-5 py-3 text-base font-semibold text-white transition-all duration-300 hover:scale-105 hover:bg-pink-600 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-offset-2"
      >
        สมัครสมาชิก
      </button>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 backdrop-blur-sm p-4">
          <div className="w-full max-w-2xl overflow-hidden rounded-[16px] bg-white shadow-[0_12px_32px_rgba(0,0,0,.08)]">
            <div className="flex items-center justify-between border-b border-[#E5E7EB] px-5 py-3">
              <div className="text-base font-semibold text-[#111827]">ข้อกำหนดการใช้งาน</div>
              <button
            ...
            <div ref={boxRef} className="max-h-[60vh] space-y-3 overflow-auto p-5 text-sm leading-6 text-[#111827]">
              <div dangerouslySetInnerHTML={{ __html: TERMS_HTML }} />
              <div className="h-8" />
              <p className="text-center text-xs text-[#6B7280]">— ถึงท้ายเอกสารแล้ว —</p>
            </div>

            <div className="flex items-center justify-between border-t border-[#E5E7EB] px-5 py-3">
              <span className="text-xs text-[#6B7280]">
                สถานะการอ่าน: {scrolledEnd ? "อ่านครบแล้ว ✅" : "ยังอ่านไม่ครบ"}
              </span>
